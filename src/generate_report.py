...
def generate_report(classifiers, best_params, metrics, cross_val_scores):
    ...
    with open(REPORT_PATH, "w") as f:
        ...
        for name in cross_val_scores.keys():
            f.write(
                f"- {name}: {cross_val_scores[name].mean():.4f} (+/- {cross_val_scores[name].std():.4f})\n"
            )
        f.write("\n")
        ...
        for name in classifiers.keys():
            ...
            f.write(
                f"![ROC Curve for {name}](reports/figures/roc_curve_{name_path}.png)\n\n"
            )

            f.write("### Precision-Recall Curve\n\n")
            f.write(f"### {name}\n\n")
            f.write(
                f"![Precision-Recall Curve for {name}](reports/figures/precision_recall_curve_{name_path}.png)\n\n"
            )
